...
'* 1.8	24/6/2023	Modify RefByHostFiles,GetMaxFileUpdateTime,GetDirSize
'********************************************************************
...
Public Class HostDir
	...
	Friend Function RefByHostFiles(InFolder As PigFolder) As String
		Dim LOG As New PigStepLog("RefByHostFiles")
		Try
			Dim strFastPigMD5 As String = ""
			Me.HostFiles.Clear()
			For Each oPigFile As PigFile In InFolder.PigFiles
				Dim strFileID As String = Me.fParent.fGetHostFileID(oPigFile.FilePath)
				With Me.HostFiles.AddOrGet(strFileID)
					.FileName = oPigFile.FileTitle
					.FileSize = oPigFile.Size
					LOG.StepName = "GetFastPigMD5"
					LOG.Ret = oPigFile.GetFastPigMD5(strFastPigMD5)
					If LOG.Ret <> "OK" Then
						LOG.AddStepNameInf(oPigFile.FilePath)
						Me.fParent.fParent.fParent.fPrintErrLogInf(LOG.StepLogInf)
						.FastPigMD5 = ""
					Else
						.FastPigMD5 = strFastPigMD5
					End If
					.FileUpdateTime = oPigFile.UpdateTime
				End With
			Next
			Dim lngCnt As Long = 0
			For Each oHostFile As HostFile In Me.HostFiles
				strFastPigMD5 &= oHostFile.FastPigMD5
	...
	Public Function GetMaxFileUpdateTime(InFolder As PigFolder) As Date
		Try
			Dim sbData As New StringBuilder("")
			For Each oFile As PigFile In InFolder.PigFiles
				sbData.Append(Me.fPigFunc.GetFmtDateTime(oFile.UpdateTime) & vbCrLf)
			Next
	...
	Public Function GetDirSize(InFolder As PigFolder) As Decimal
		Try
			GetDirSize = 0
			For Each oFile As PigFile In InFolder.PigFiles
	...
	Public Function GetAvgFileUpdateTime(InFolder As PigFolder) As Date
		Try
			Dim decTotalSec As Decimal = 0, lngCnt As Long = 0
			Dim dteNow As Date = Now
			For Each oFile As PigFile In InFolder.PigFiles
				lngCnt += 1
				decTotalSec += DateDiff(DateInterval.Second, oFile.UpdateTime, dteNow)
			Next
